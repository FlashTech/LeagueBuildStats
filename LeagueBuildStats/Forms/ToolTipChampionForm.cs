...
{
	public partial class ToolTipChampionForm : Form
	{
		...
		public ToolTipChampionForm(int x, int y, ItemControl itemcontrol, CreateItemDiv item)
			   : this()
		{
		...
		public ToolTipChampionForm()
		{
			...
			picBoxChampion.Click += picBoxItem_Click;
		...
		public void UpdateToolTipForm(Image image, string itemName, int itemId, string price, string htmlString, string version)
		{
			...
			//tempName = tempName.Replace(">", "");
			//tempName = tempName.Replace("|", "");
			string itemFile = localFolder + string.Format(@"\Data\Items\ItemInfo{0}.{1}.htm", itemId, version);
